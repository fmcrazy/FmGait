...
def init_seeds(seed=0, cuda_deterministic=True):
    #  随机种子函数，可以确保在进行深度学习训练时，生成的随机数具有可复现性。这对于实验的稳定性和结果的一致性非常重要。
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    # Speed-reproducibility tradeoff https://pytorch.org/docs/stable/notes/randomness.html
    if cuda_deterministic:  # slower, more reproducible， 更慢，更有重复性
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False
    else:  # faster, less reproducible， 更快，少的重复性
...
def get_ddp_module(module, **kwargs):
    ...
    module = DDPPassthrough(module, device_ids=[device], output_device=device,   # 指定输入设备和输出设备

...
class MessageManager:
    def __init__(self):
        ...
        self.writer_hparams = ['image', 'scalar']  # 图像，标量
        self.time = time.time()
    ...
    def init_logger(self, save_path, log_to_file):
        # init logger
        self.logger = logging.getLogger('opengait')  # 获取一个叫opengait的记录器
        self.logger.setLevel(logging.INFO)   # 设置默认的日志级别为info
        self.logger.propagate = False
        formatter = logging.Formatter(
            fmt='[%(asctime)s] [%(levelname)s]: %(message)s', datefmt='%Y-%m-%d %H:%M:%S') # 设置日志输出的格式
        if log_to_file:
            ...
            self.logger.addHandler(vlog)  # 将日志输出到文件

        console = logging.StreamHandler()
        console.setFormatter(formatter)
        console.setLevel(logging.DEBUG)
        self.logger.addHandler(console)  # 将日志输出到控制台

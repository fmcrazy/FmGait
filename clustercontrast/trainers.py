...
class ClusterContrastTrainer(object):
    ...
    def train(self, args, epoch, data_loader, optimizer, pseudo_labeled_dataset, refinement_pseudo_labeled_dataset
              , part_score, print_freq, train_iters):
        self.encoder.train()
        ...
        for i in range(train_iters):
            ...
            # process inputs
            ipts, ema_ipts, labels, fnames, l, part_ipts, score = self._inputs_pretreament(inputs, pseudo_labeled_dataset,
                                                                               refinement_pseudo_labeled_dataset, part_score)
            score = score[0:l,:]
            f_out, part_outs= self._forward(ipts)
            part_out = compute_part_feature(part_outs, part_num=4)
            with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                # ema_out = self.ema_forward(part_ipts)

            memory_loss = self.memory(epoch, f_out, ema_out, part_out, score, labels, refinement_labels_p, args.use_refine_label, args.use_aug)
            del refinement_labels, refinement_labels_p, ema_out
            # 总的损失
            loss = memory_loss
            optimizer.zero_grad()
            loss.backward()
    ...
    def _inputs_pretreament(self, inputs, pseudo_labeled_dataset, refinement_pseudo_labeled_dataset, part_score):
        ...
        part_ipts = self.encoder.q_inputs_pretreament(inputs, use_leg=True, length=l)
        ...
        fnames = inputs[4]
        return ipts, ema_ipts, labels.cuda(), fnames, l, part_ipts, score.cuda()
    ...
    def _forward(self, inputs):
        ...
        return outputs, part_outs

    def _part_forward(self, inputs):
        with torch.no_grad():
            with autocast(enabled=self.encoder.engine_cfg['enable_float16']):
                retval = self.encoder(inputs)
                inference_feat = retval['inference_feat']

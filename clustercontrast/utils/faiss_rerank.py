...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False):
    ...
    ngpus = faiss.get_num_gpus()  # 使用的GPU的数量
    ...
    if (search_option==0):
        ...
        initial_rank = initial_rank.cpu().numpy()  # numpy数组一般放在cpu上训练
    ...
    for i in range(N):
        ...
        for candidate in k_reciprocal_index:
            ...
            if (len(np.intersect1d(candidate_k_reciprocal_index,k_reciprocal_index)) > 1/3*len(candidate_k_reciprocal_index)):   # 修改：将前面的系数由2/3改为1/2
                k_reciprocal_expansion_index = np.append(k_reciprocal_expansion_index,candidate_k_reciprocal_index)

        k_reciprocal_expansion_index = np.unique(k_reciprocal_expansion_index)  # element-wise unique, 去除数组中重复的元素

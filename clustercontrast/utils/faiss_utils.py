...
def search_raw_array_pytorch(res, xb, xq, k, D=None, I=None,  # xb和xq都表示目标特征
                             metric=faiss.METRIC_L2):
    assert xb.device == xq.device

    nq, d = xq.size()   # nq是样本的个数， d是特征的维度
    if xq.is_contiguous():  # 是否是连续存储的
        xq_row_major = True
    ...
    xq_ptr = swig_ptr_from_FloatTensor(xq)  # 将内存的指针转化为浮点数指针，并考虑的偏移情况
